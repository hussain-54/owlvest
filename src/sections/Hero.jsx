...
export default function Hero() {
  ...
  const presaleEnd = new Date(Date.now() + 18 * 24 * 60 * 60 * 1000); // 18 days from now
        ...
        <div className="bg-[#1B0E3F] w-full rounded-xl p-6 text-center shadow-md border border-[#2F1C55]">
          ...
          <div className="flex justify-center gap-6 text-black">
            {['Days', 'Hours', 'Minutes', 'Seconds'].map((label, i) => (
              <div key={i} className="bg-[#00F08F] w-20 h-20 sm:w-24 sm:h-24 rounded-lg flex flex-col justify-center items-center font-bold text-xl sm:text-2xl">
                <Countdown
                  date={presaleEnd}
